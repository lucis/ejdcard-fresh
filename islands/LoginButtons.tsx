import { useCallback, useMemo } from "preact/hooks";
import {
  getSupabaseClient,
  DEFAULT_RETURN_URL,
  RETURN_URL_QUERY_PARAM,
} from "../auth.ts";

const client = getSupabaseClient();

export default function LoginButtons() {
  const redirectTo = useMemo(() => {
    const query = new URLSearchParams(window.location?.search);
    const providedReturnUrl = query.get(RETURN_URL_QUERY_PARAM);
    const returnUrl = providedReturnUrl || DEFAULT_RETURN_URL;

    const successQueryParams = new URLSearchParams();
    successQueryParams.set(RETURN_URL_QUERY_PARAM, returnUrl);

    return (
      window.location?.protocol +
      "//" +
      window.location?.host +
      "/login/success?" +
      successQueryParams.toString()
    );
  }, []);

  const oAuthLogin = useCallback((provider: "google" | "github") => {
    // {
    //   shouldCreateUser: true,
    //   redirectTo,
    // }
    client.auth.signInWithOAuth({
      provider,
      options: {
        redirectTo,
      },
    });
  }, []);

  return (
    <div>
      <div class="absolute bottom-1/4 px-32 context">
        <div class="text-8xl font-medium font-title dark:text-primary-light">
          Log in
        </div>
        <div class="mt-5 mb-20 text-xl font-title dark:text-primary-light">
          Welcome back. Let's rock!
        </div>
        <div>
          <button
            class="transition duration-300 ease-out hover:ease-in rounded bg-primary-light border-2 border-primary-light hover:border-[#113131] flex items-center justify-center w-72 py-3 mt-3"
            type="button"
            onClick={() => oAuthLogin("google")}
          >
            <svg
              version="1.1"
              x="0px"
              y="0px"
              viewBox="0 0 48 48"
              enable-background="new 0 0 48 48"
              height="1.2em"
              width="1.2em"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fill="#FFC107"
                d="M43.611,20.083H42V20H24v8h11.303c-1.649,4.657-6.08,8-11.303,8c-6.627,0-12-5.373-12-12
	c0-6.627,5.373-12,12-12c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657C34.046,6.053,29.268,4,24,4C12.955,4,4,12.955,4,24
	c0,11.045,8.955,20,20,20c11.045,0,20-8.955,20-20C44,22.659,43.862,21.35,43.611,20.083z"
              ></path>
              <path
                fill="#FF3D00"
                d="M6.306,14.691l6.571,4.819C14.655,15.108,18.961,12,24,12c3.059,0,5.842,1.154,7.961,3.039l5.657-5.657
	C34.046,6.053,29.268,4,24,4C16.318,4,9.656,8.337,6.306,14.691z"
              ></path>
              <path
                fill="#4CAF50"
                d="M24,44c5.166,0,9.86-1.977,13.409-5.192l-6.19-5.238C29.211,35.091,26.715,36,24,36
	c-5.202,0-9.619-3.317-11.283-7.946l-6.522,5.025C9.505,39.556,16.227,44,24,44z"
              ></path>
              <path
                fill="#1976D2"
                d="M43.611,20.083H42V20H24v8h11.303c-0.792,2.237-2.231,4.166-4.087,5.571
	c0.001-0.001,0.002-0.001,0.003-0.002l6.19,5.238C36.971,39.205,44,34,44,24C44,22.659,43.862,21.35,43.611,20.083z"
              ></path>
            </svg>
            <div class="ml-2 text-xl">Log in with Gmail</div>
          </button>
        </div>
      </div>
    </div>
  );
}
